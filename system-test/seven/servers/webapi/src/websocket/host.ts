...
import { LocationPaths } from '@seven/core';
...
export class Host {
    ...
    public constructor(
        ...
        this.sessionProvider = this.generateSessionProvider(request);
        this.setBinds();
    }
    ...
    private generateSessionProvider(request: http.IncomingMessage): DefinedProvider {
        const context = [ this.guid, request ];
        const session = this.service.injector.get(Session, true, undefined, context);
        return this.sessionProvider = new DefinedProvider(Session, session);
    }
    ...
    private getPath(path: string, profile?: string, app?: string): any {
        const pathObject = this.service.getPath(path);
        const customProviders: IProvider[] = [
            this.sessionProvider,
            new DefinedProvider(LocationPaths, new LocationPaths(profile, app))
        ];
        return this.service.injector.get(pathObject, true, customProviders);
    }
    ...
    private processMsg(msg: IMessage): boolean {
        if (msg.method) {
            ...
            try {
                const path = this.getPath(msg.path, msg.profile, msg.app);
