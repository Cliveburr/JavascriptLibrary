...
export class MemoryService {
    ...
    async createMemory(userId: string, name: string): Promise<MemoryEntity> {
        if (!userId?.trim()) {
    ...
    async getMemoriesByUser(userId: string): Promise<MemoryEntity[]> {
        if (!userId?.trim()) {
            throw new MemoryValidationError('User ID is required');
        }
    ...
    async updateMemory(id: string, name: string): Promise<MemoryEntity | null> {
        if (!id?.trim()) {
            throw new MemoryValidationError('Memory ID is required');
        }
